...
namespace Sunmking\Think8Vite;

use Exception;
use Sunmking\Think8Vite\collections\Think8Collection;
use Sunmking\Think8Vite\exception\ViteManifestNotFoundException;
use Sunmking\Think8Vite\support\Htmlable;
use Sunmking\Think8Vite\support\HtmlString;
use Sunmking\Think8Vite\traits\Macroable;
...
class Vite implements Htmlable
{
    ...
    {
        $entrypoints = think8_collect($entrypoints);
        ...
        foreach ($entrypoints as $entrypoint) {
            ...
            $preloads->push([
                $chunk['src'],
                $this->think8AssetPath("{$buildDirectory}/{$chunk['file']}"),
                $chunk,
                $manifest,
            ]);
            ...
            foreach ($chunk['imports'] ?? [] as $import) {
                $preloads->push([
                    $import,
                    $this->think8AssetPath("{$buildDirectory}/{$manifest[$import]['file']}"),
                    $manifest[$import],
                    $manifest,
                ]);

                foreach ($manifest[$import]['css'] ?? [] as $css) {
                    $partialManifest = Think8Collection::make($manifest)->where('file', $css);

                    $preloads->push([
                        $partialManifest->keys()->first(),
                        $this->think8AssetPath("{$buildDirectory}/{$css}"),
    ...
    protected function makePreloadTagForChunk($src, $url, $chunk, $manifest): string
    {
        ...
        $this->preloadedAssets[$url] = $this->parseAttributes(
            Think8Collection::make($attributes)->forget('href')->all()
        );
    ...
    {
        return Think8Collection::make($attributes)
     ...
     */
    public function reactRefresh(): HtmlString|null
    {
        if (! $this->isRunningHot()) {
            return null;
        }
     ...
     * @throws Exception
     */
    public function think8Asset($asset, $buildDirectory = null): string
    {
        $buildDirectory ??= $this->buildDirectory;
        ...
        return $this->think8AssetPath($buildDirectory.'/'.$chunk['file']);
    }
     ...
     * @return string
     */
    protected function think8AssetPath($path, $secure = null): string
    {
        return think8_asset($path, $secure);
    }
